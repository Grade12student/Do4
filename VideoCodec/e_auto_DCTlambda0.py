...
def apply_transform(image, transform_type='dct'):
...
def encoder(loadmodel, input_folder, outputfolder, bit_depth=8, lambda_factor=0.1):
    ...
    for i in range(0, num_frames - 2):
        input_frame_file = frame_files[i]
        refer_frame_file = frame_files[i + 2]

        input_frame_path = os.path.join(input_folder, frame_files[i])
        refer_frame_path = os.path.join(input_folder, frame_files[i+2])
        
        im1 = imageio.imread(input_frame_path)
        im2 = imageio.imread(refer_frame_path)

        output_folder_frame = os.path.join(outputfolder, f"frame_{i + 1}")
    ...
    input_image_name = os.path.basename(input_frame_path)
    refer_image_name = os.path.basename(refer_frame_path)
    ...
    print(f"Processing {input_image_name} with reference {refer_image_name}: BPP = {bitrate_cs}, PSNR = {distortion_cs}")
    ...
    return bitrate_cs, distortion_cs
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_folder', type=str, dest="input_folder", default='./frames', help="input frames folder")
    parser.add_argument('--output_folder', type=str, dest="outputfolder", default='./testpkl/F/', help="output pkl folder")
    parser.add_argument('--transform_type', type=str, dest="transform_type", default='dct', help="sparsity-inducing transform type")
    parser.add_argument('--lambda_factor', type=float, dest="lambda_factor", default=0.1, help="lambda factor for bitrate estimation")
    ...
    psnr_values = []  # Store PSNR values for each pair of images

    for i in range(len(input_images) - 2):
        input_frame_path = os.path.join(input_folder, input_images[i])
        refer_frame_path = os.path.join(input_folder, input_images[i + 2])

        im1 = imageio.imread(input_frame_path)
        im1 = im1 / 255.0
        im1 = np.expand_dims(im1, axis=0)  # Add batch dimension

        # Apply the chosen sparsity-inducing transform
        ...
        output_folder_frame = os.path.join(args.outputfolder, f"frame_{i + 1}")
       

        # Call the encoder function
        bitrate_cs, distortion_cs = encoder(args.loadmodel, input_folder, output_folder_frame, measurement_matrix, args.lambda_factor)
        ...
        # Store PSNR value for later printing
        psnr_values.append(distortion_cs)

    # Print PSNR values outside the loop
