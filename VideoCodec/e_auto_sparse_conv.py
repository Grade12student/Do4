...
def apply_transform(image, transform_type='sparse_conv'):
    if transform_type == 'dct':
        ...
        # Apply sparsity to the weights
        sparsity_factor = 0.9  # Adjust as needed
        for layer in model.layers:
            if isinstance(layer, tf.keras.layers.Conv2D):
                weights = layer.get_weights()
                weights[0][np.abs(weights[0]) < np.percentile(np.abs(weights[0]), sparsity_factor * 100)] = 0
